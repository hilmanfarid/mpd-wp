...
class sd_controller extends wbController{    
    ...
    public static function read($args = array()){
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'sd_id');
		...
		$sd_keterangan = wbRequest::getVarClean('sd_keterangan', 'str', '');
		$sd_type = wbRequest::getVarClean('sd_type', 'str', '');

        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 'sd');
    ...
    */
    public static function upload_excel($args = array()){
        ...
        try {
            ...
            $errorMessage = '';
            /* cek data sd */
            $tahunAjaranSDExist = $table->isExistTahunAjaran($items[0]['sd_id'], $items[0]['sd_det_thn_ajaran']);
            if(!$tahunAjaranSDExist) {
                $items[0][$table->pkey] = $table->GenID();
                $table->setRecord($items[0]);  
                $table->create();      
            }else {
                $errorMessage .= '- Data SD Tahun Ajaran '.$items[0]['sd_det_thn_ajaran'].' tidak dapat diinput karena sudah ada <br>';    
            }
            
            
            /* cek data mi */
            $tahunAjaranMIExist = $table->isExistTahunAjaran($items[1]['sd_id'], $items[1]['sd_det_thn_ajaran']);
            if(!$tahunAjaranMIExist) {
                $items[1][$table->pkey] = $table->GenID();
                $table->setRecord($items[1]);  
                $table->create();      
            }else {
                $errorMessage .= '- Data MI Tahun Ajaran '.$items[1]['sd_det_thn_ajaran'].' tidak dapat diinput karena sudah ada <br>';    
            }
