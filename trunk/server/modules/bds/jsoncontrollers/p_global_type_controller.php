...
/**
 * p_global_type
 * class controller for table bds_p_global_type 
 *
 * @since 23-10-2012 12:07:20
 * @author wiliamdecosta@gmail.com
 */
class p_global_type_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('PGlobalType')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'gtype_id');
        $dir = wbRequest::getVarClean('dir', 'str', 'DESC');
        $query = wbRequest::getVarClean('query', 'str', '');
        
        $gtype_id = wbRequest::getVarClean('gtype_id', 'int', 0);
		$gtype_code = wbRequest::getVarClean('gtype_code', 'str', '');
		$gtype_description = wbRequest::getVarClean('gtype_description', 'str', '');
		$gtype_creation_date = wbRequest::getVarClean('gtype_creation_date', 'date', '');
		$gtype_creation_by = wbRequest::getVarClean('gtype_creation_by', 'str', '');
		$gtype_updated_date = wbRequest::getVarClean('gtype_updated_date', 'date', '');
		$gtype_updated_by = wbRequest::getVarClean('gtype_updated_by', 'str', '');
		
		$searchKode = wbRequest::getVarClean('searchKode', 'str', '');

        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 'p_global_type');
            ...
            if(!empty($searchKode)) {
                $table->setCriteria('gtype_code ILIKE ?', array('%'.$searchKode.'%'));
            }
    ...
    public static function create($args = array()){
        ...
        $details_p_global_param = wbRequest::getVarClean('details_p_global_param', 'str', '');
		$details_p_global_param =& wbUtil::jsonDecode($details_p_global_param);
		if (!is_array($details_p_global_param)) $details_p_global_param = array();
        ...
        if (isset($items[0])){
	        ...
	        try{
                ...
                $p_global_param =& wbModule::getModel('bds', 'p_global_param');
                $p_global_param->actionType = 'CREATE';
				foreach ($details_p_global_param as $detail){
				    // generate nextID using master table connection
					$detail[$p_global_param->pkey] = $table->dbconn->GenID($p_global_param->table);
					$detail[$table->pkey] = $items[$table->pkey];
					$p_global_param->setRecord($detail);
					// create detail using master table connection
					$p_global_param->create($table->dbconn);
				}
