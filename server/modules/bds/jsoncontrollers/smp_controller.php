...
class smp_controller extends wbController{    
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('Smp')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'smp_id');
        $dir = wbRequest::getVarClean('dir', 'str', 'ASC');
        $query = wbRequest::getVarClean('query', 'str', '');
        
        $smp_id = wbRequest::getVarClean('smp_id', 'int', 0);
		$smp_tahun_ajaran = wbRequest::getVarClean('smp_tahun_ajaran', 'str', '');
		$smp_keterangan = wbRequest::getVarClean('smp_keterangan', 'str', '');
		$smp_type = wbRequest::getVarClean('smp_type', 'str', '');

        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 'smp');
