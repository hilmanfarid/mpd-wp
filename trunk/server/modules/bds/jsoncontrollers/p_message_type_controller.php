...
/**
 * p_message_type
 * class controller for table bds_p_message_type 
 *
 * @since 23-10-2012 12:07:20
 * @author hilman farid
 */
class p_message_type_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('p_message_type')) return;
        ...
        try{
            ...
            $items = $table->dbconn->GetAll("select msg_type.* from p_message_type msg_type
                                            left join p_msg_type_role_map rol_msg on rol_msg.p_message_type_id = msg_type.p_message_type_id
                                            where p_app_role_id_for = 17");
            $total = $table->dbconn->GetOne("select count(*) from p_message_type msg_type
                                            left join p_msg_type_role_map rol_msg on rol_msg.p_message_type_id = msg_type.p_message_type_id
                                            where p_app_role_id_for = 17");
    ...
    public static function create($args = array()){
        ...
        $table =& wbModule::getModel('bds', 'p_message_type');
