...
/**
 * p_job_position
 * class controller for table bds_p_job_position 
 *
 * @since 23-10-2012 12:07:20
 * @author wiliamdecosta@gmail.com
 */
class p_job_position_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('PJobPosition')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'jobpos_listing_no');
        $dir = wbRequest::getVarClean('dir', 'str', 'ASC');
        $query = wbRequest::getVarClean('query', 'str', '');
        
        $jobpos_id = wbRequest::getVarClean('jobpos_id', 'int', 0);
		$jobpos_code = wbRequest::getVarClean('jobpos_code', 'str', '');
		$jobpos_description = wbRequest::getVarClean('jobpos_description', 'str', '');
		$jobpos_status = wbRequest::getVarClean('jobpos_status', 'str', '');
		
		$jobpos_creation_date = wbRequest::getVarClean('jobpos_creation_date', 'date', '');
		$jobpos_creation_by = wbRequest::getVarClean('jobpos_creation_by', 'str', '');
		$jobpos_updated_date = wbRequest::getVarClean('jobpos_updated_date', 'date', '');
		$jobpos_updated_by = wbRequest::getVarClean('jobpos_updated_by', 'str', '');
        
        $searchKodePosisi = wbRequest::getVarClean('searchKodePosisi', 'str', '');
            
        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 'p_job_position');
            ...
            if(!empty($searchKodePosisi)) {
                $table->setCriteria('jobpos_code ILIKE ?', array('%'.$searchKodePosisi.'%'));
            }
