...
/**
 * p_school_level
 * class controller for table bds_p_school_level 
 *
 * @since 23-10-2012 12:07:20
 * @author wiliamdecosta@gmail.com
 */
class p_school_level_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('P_School_Level')) return;
        ...
        $p_school_level_id = wbRequest::getVarClean('p_school_level_id', 'int', 0);
        ...
        try{
            $table =& wbModule::getModel('bds', 'p_school_level');
            ...
            if(!empty($searchKode)) {
                $table->setCriteria('code ILIKE ?', array('%'.$searchKode.'%'));
            }
    ...
    public static function create($args = array()){
        ...
        if (!is_array($items)){
            $data['message'] = 'Invalid items school level';
            return $data;
        }
        $details_p_school_type = wbRequest::getVarClean('details_p_school_type', 'str', '');
		$details_p_school_type =& wbUtil::jsonDecode($details_p_school_type);
		if (!is_array($details_p_school_type)) $details_p_school_type = array();
        ...
        if (isset($items[0])){
	        ...
	        try{
                ...
                $p_school_type =& wbModule::getModel('bds', 'p_school_type');
                $p_school_type->actionType = 'CREATE';
				foreach ($details_p_school_type as $detail){
				    // generate nextID using master table connection
					$detail[$p_school_type->pkey] = $table->dbconn->GenID($p_school_type->table);
					$detail[$table->pkey] = $items[$table->pkey];
					$p_school_type->setRecord($detail);
					// create detail using master table connection
					$p_school_type->create($table->dbconn);
				}
