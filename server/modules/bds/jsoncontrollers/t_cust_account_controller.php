...
/**
 * t_cust_account
 * class controller for table bds_t_cust_account 
 *
 * @since 23-10-2012 12:07:20
 * @author hilman farid
 */
class t_cust_account_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        ...
        $t_customer_id = wbRequest::getVarClean('t_customer_id', 'int', 0);
        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 't_cust_account');
            ...
            $table->setCriteria("t_customer_id = ?",array($t_customer_id));
            if(!empty($t_cust_account_id)){
                $table->setCriteria("t_cust_account_id = ?",array($t_cust_account_id));
            }
            $query = "SELECT * FROM v_cust_account_update ".$table->getCriteriaSQL()." ORDER BY $sort $dir";
        	$items = $table->dbconn->GetAllAssocLimit($query,$limit,$start);
        	$query = '';
        	$query = "SELECT COUNT(1) FROM v_cust_account_update ".$table->getCriteriaSQL();
