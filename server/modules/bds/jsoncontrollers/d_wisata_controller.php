...
/**
 * d_wisata
 * class controller for table bds_d_wisata 
 *
 * @since 13-12-2012 20:06:34
 * @author agung.hp
 */
class d_wisata_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('DWisata')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'wisata_listing_no');
		...
		$wisata_code = wbRequest::getVarClean('wisata_code', 'str', '');
		$wisata_name = wbRequest::getVarClean('wisata_name', 'str', '');
		$wisata_address_1 = wbRequest::getVarClean('wisata_address_1', 'str', '');
		$wisata_address_2 = wbRequest::getVarClean('wisata_address_2', 'str', '');
		$wisata_kota = wbRequest::getVarClean('wisata_kota', 'str', '');
		$wisata_kode_pos = wbRequest::getVarClean('wisata_kode_pos', 'str', '');
		$wisata_phone_no = wbRequest::getVarClean('wisata_phone_no', 'str', '');
		$wisata_website = wbRequest::getVarClean('wisata_website', 'str', '');
		$wisata_listing_no = wbRequest::getVarClean('wisata_listing_no', 'int', 0);
		$wisata_description = wbRequest::getVarClean('wisata_description', 'str', '');
        ...
        try{
            $table =& wbModule::getModel('bds', 'd_wisata');
    ...
    public static function create($args = array()){
        ...
        $details_t_wisata_detail = wbRequest::getVarClean('details_t_wisata_detail', 'str', '');
		$details_t_wisata_detail =& wbUtil::jsonDecode($details_t_wisata_detail);
		if (!is_array($details_t_wisata_detail)) $details_t_wisata_detail = array();
        ...
        if (isset($items[0])){
	        ...
	        try{
                ...
                $t_wisata_detail =& wbModule::getModel('bds', 't_wisata_detail');
                $t_wisata_detail->actionType = 'CREATE';
				foreach ($details_t_wisata_detail as $detail){
				    // generate nextID using master table connection
					$detail[$t_wisata_detail->pkey] = $table->dbconn->GenID($t_wisata_detail->table);
					$detail[$table->pkey] = $items[$table->pkey];
					$t_wisata_detail->setRecord($detail);
					// create detail using master table connection
					$t_wisata_detail->create($table->dbconn);
				}
