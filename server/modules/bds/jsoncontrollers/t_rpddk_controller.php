...
/**
 * t_rpddk
 * class controller for table bds_t_rpddk 
 *
 * @since 13-12-2012 20:06:34
 * @author agung.hp
 */
class t_rpddk_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('TRpddk')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 't_rpddk_id');
        $dir = wbRequest::getVarClean('dir', 'str', 'ASC');
        $query = wbRequest::getVarClean('query', 'str', '');
        
        $t_rpddk_id = wbRequest::getVarClean('t_rpddk_id', 'int', 0);
		$jenisdata = wbRequest::getVarClean('jenisdata', 'int', 0);
		$kelompok_id = wbRequest::getVarClean('kelompok_id', 'int', 0);
		$tahun = wbRequest::getVarClean('tahun', 'str', '');
		$laki = wbRequest::getVarClean('laki', 'str', '');
		$perempuan = wbRequest::getVarClean('perempuan', 'str', '');
        ...
        try{
            $table =& wbModule::getModel('bds', 't_rpddk');
        ...
        if (isset($items[0])){
	        ...
	        try{
	            ...
	            $table->dbconn->BeginTrans();
    	            ...
    	            $data['success'] = true;
    	            $data['message'] = 'Data berhasil disimpan';
    	            $data['items'] = array($table->get($items[$table->pkey]));
    ...
    public static function update($args = array()){
        ...
        if (isset($items[0])){
	        ...
	        $data['items'] = array_merge($items, $table->get($items[$table->pkey]));
        }
