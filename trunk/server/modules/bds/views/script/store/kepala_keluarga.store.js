/**
 * @class Bds.store.kepala_keluarga
 * Store for table bds_kepala_keluarga
 *
 * @author agung.hp
 * @since 31-10-2012 11:02:06
 */
Bds.store.kepala_keluarga = function(config){
	var config = config || {};
	Ext.applyIf(config, {
        proxy: new Ext.data.HttpProxy({
            api: {
                read : Webi.ROUTE_URL + '&class=d_warga&method=read',
	...
	Bds.store.kepala_keluarga.superclass.constructor.call(this, config);
};

Ext.extend(Bds.store.kepala_keluarga, Ext.data.Store);
Ext.reg('store_kepala_keluarga', Bds.store.kepala_keluarga);