...
class user_permission_controller extends wbController{    
    ...
    public static function read($args = array()){
        ...
        try{
            $table =& wbModule::getModel('base', 'roles.user_permission');
        ...
        if (isset($items[0])){
        	...
        	for($i=0; $i < $numItems; $i++){
        		try{
        			...
        			$items[$i] = $table->get($table->record['user_id'], $table->record['permission_id']);
        		}catch(Exception $e){
	        ...
	        try{
	            ...
	            $table->dbconn->CommitTrans();
                $data['items'] = $table->get($table->record['user_id'], $table->record['permission_id']);
	        }catch (Exception $e) {
            ...
            $table->dbconn->BeginTrans();
                if (is_array($items)){
                    foreach ($items as $key => $value){
                        ...
                        $data['items'][] = array('user_permission_id' => $value);
                        $data['total']++;
                    }
                }else{
                    if (empty($items)){
                    ...
                    $table->remove($keys[0], $keys[1]);
                    $data['items'][] = array('user_permission_id' => $items);
