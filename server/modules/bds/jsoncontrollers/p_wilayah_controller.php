...
/**
 * p_wilayah
 * class controller for table bds_p_wilayah 
 *
 * @since 23-10-2012 12:07:20
 * @author wiliamdecosta@gmail.com
 */
class p_wilayah_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('PWilayah')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'p_wilayah.wilayah_kode');
		...
		$wilayah_kode = wbRequest::getVarClean('wilayah_kode', 'str', '');
		$wilayah_nama = wbRequest::getVarClean('wilayah_nama', 'str', '');
		$wilayah_description = wbRequest::getVarClean('wilayah_description', 'str', '');
		$wilayah_status = wbRequest::getVarClean('wilayah_status', 'str', '');
		$wilayah_creation_date = wbRequest::getVarClean('wilayah_creation_date', 'date', '');
		$wilayah_creation_by = wbRequest::getVarClean('wilayah_creation_by', 'str', '');
		$wilayah_updated_date = wbRequest::getVarClean('wilayah_updated_date', 'date', '');
		$wilayah_updated_by = wbRequest::getVarClean('wilayah_updated_by', 'str', '');
        
        $searchWilayah = wbRequest::getVarClean('searchWilayah', 'str', '');
        $comboDisplay = wbRequest::getVarClean('comboDisplay', 'str', '');
        
        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 'p_wilayah');
            ...
            if(!empty($searchWilayah)) {
                $table->setCriteria('(p_wilayah.wilayah_kode ILIKE ? OR p_wilayah.wilayah_nama ILIKE ?)', array('%'.$searchWilayah.'%', '%'.$searchWilayah.'%'));
            }
            
            if($comboDisplay == 'Y') {
                $table->setCriteria('p_wilayah.wilayah_status != ?', array('61'));    
            }
