...
/**
 * d_skpd
 * class controller for table bds_d_skpd 
 *
 * @since 13-12-2012 14:15:12
 * @author agung.hp
 */
class d_skpd_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('DSkpd')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'd_skpd_id');
        $dir = wbRequest::getVarClean('dir', 'str', 'DESC');
        $query = wbRequest::getVarClean('query', 'str', '');
        
        $d_skpd_id = wbRequest::getVarClean('d_skpd_id', 'int', 0);
		$code = wbRequest::getVarClean('code', 'str', '');
		$skpd_name = wbRequest::getVarClean('skpd_name', 'str', '');
		...
		$phone_no = wbRequest::getVarClean('phone_no', 'str', '');
		$website = wbRequest::getVarClean('website', 'str', '');
        ...
        try{
            $table =& wbModule::getModel('bds', 'd_skpd');
    ...
    public static function create($args = array()){
        ...
        $details_t_skpd_employee = wbRequest::getVarClean('details_t_skpd_employee', 'str', '');
		$details_t_skpd_employee =& wbUtil::jsonDecode($details_t_skpd_employee);
		if (!is_array($details_t_skpd_employee)) $details_t_skpd_employee = array();
        ...
        if (isset($items[0])){
	        ...
	        try{
                ...
                $t_skpd_employee =& wbModule::getModel('bds', 't_skpd_employee');
                $t_skpd_employee->actionType = 'CREATE';
				foreach ($details_t_skpd_employee as $detail){
				    // generate nextID using master table connection
					$detail[$t_skpd_employee->pkey] = $table->dbconn->GenID($t_skpd_employee->table);
					$detail[$table->pkey] = $items[$table->pkey];
					$t_skpd_employee->setRecord($detail);
					// create detail using master table connection
					$t_skpd_employee->create($table->dbconn);
				}
