...
/**
 * d_puskesmas
 * class controller for table bds_d_puskesmas 
 *
 * @since 06-12-2012 11:58:41
 * @author agung.hp
 */
class d_puskesmas_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('DPuskesmas')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'puskesmas_id');
        $dir = wbRequest::getVarClean('dir', 'str', 'DESC');
        $query = wbRequest::getVarClean('query', 'str', '');
        
        $puskesmas_id = wbRequest::getVarClean('puskesmas_id', 'int', 0);
		$kecamatan_id = wbRequest::getVarClean('kecamatan_id', 'int', 0);
		$puskesmas_kode = wbRequest::getVarClean('puskesmas_kode', 'str', '');
		$puskesmas_name = wbRequest::getVarClean('puskesmas_name', 'str', '');
		$puskesmas_alamat = wbRequest::getVarClean('puskesmas_alamat', 'str', '');
		$puskesmas_kode_pos = wbRequest::getVarClean('puskesmas_kode_pos', 'str', '');
		$puskesmas_description = wbRequest::getVarClean('puskesmas_description', 'str', '');
		$puskesmas_creation_date = wbRequest::getVarClean('puskesmas_creation_date', 'date', '');
		$puskesmas_created_by = wbRequest::getVarClean('puskesmas_created_by', 'str', '');
		$puskesmas_updated_date = wbRequest::getVarClean('puskesmas_updated_date', 'date', '');
		$puskesmas_updated_by = wbRequest::getVarClean('puskesmas_updated_by', 'str', '');
		
		$searchPuskesmas = wbRequest::getVarClean('searchPuskesmas', 'str', '');

        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 'd_puskesmas');
            ...
            if(!empty($searchPuskesmas)) {
                $table->setCriteria('(d_puskesmas.puskesmas_kode ILIKE ? OR d_puskesmas.puskesmas_name ILIKE ?)', array('%'.$searchPuskesmas.'%', '%'.$searchPuskesmas.'%'));    
            }
