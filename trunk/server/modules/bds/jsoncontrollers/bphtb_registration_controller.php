...
class bphtb_registration_controller extends wbController{    
    ...
    public static function read($args = array()){
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 't_bphtb_registration_id');
        $dir = wbRequest::getVarClean('dir', 'str', 'ASC');
        $query = wbRequest::getVarClean('query', 'str', '');

        $t_bphtb_registration_id = wbRequest::getVarClean('t_bphtb_registration_id', 'int', 0);
		$receipt_no = wbRequest::getVarClean('receipt_no', 'str', '');
		$registration_no = wbRequest::getVarClean('registration_no', 'str', '');
		$njop_pbb = wbRequest::getVarClean('njop_pbb', 'str', '');
		$wp_name = wbRequest::getVarClean('wp_name', 'str', '');
        ...
        try{
        	...
        	$table =& wbModule::getModel('bds', 'bphtb_registration');
        	...
        	$query = $table->selectClause.' '.$table->fromClause.' '.$table->getCriteriaSQL();
        	$items = $dbConn_rwnet->GetAllAssocLimit($query,$limit,$start);
        	$query = "SELECT COUNT(1) ". $table->fromClause ." ".$table->getCriteriaSQL();
            $countitems = $dbConn_rwnet->GetOne($query);
            if ($countitems === false){
