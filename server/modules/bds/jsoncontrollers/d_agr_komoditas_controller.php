...
class d_agr_komoditas_controller extends wbController{    
    ...
    public static function read($args = array()){
        ...
        $d_agr_komiditas_id = wbRequest::getVarClean('d_agr_komiditas_id', 'int', 0);
		$type_id = wbRequest::getVarClean('type_id', 'int', 0);
		$code = wbRequest::getVarClean('code', 'str', '');
		$komoditas_name = wbRequest::getVarClean('komoditas_name', 'str', '');
        ...
        try{
            $table =& wbModule::getModel('bds', 'd_agr_komoditas');
    ...
    public static function create($args = array()){
        ...
        $details_t_agr_komod_prod = wbRequest::getVarClean('details_t_agr_komod_prod', 'str', '');
		$details_t_agr_komod_prod =& wbUtil::jsonDecode($details_t_agr_komod_prod);
		if (!is_array($details_t_agr_komod_prod)) $details_t_agr_komod_prod = array();
        ...
        if (isset($items[0])){
	        ...
	        try{
                ...
                $t_agr_komod_prod =& wbModule::getModel('bds', 't_agr_komod_prod');
                $t_agr_komod_prod->actionType = 'CREATE';
				foreach ($details_t_agr_komod_prod as $detail){
				    // generate nextID using master table connection
					$detail[$t_agr_komod_prod->pkey] = $table->dbconn->GenID($t_agr_komod_prod->table);
					$detail[$table->pkey] = $items[$table->pkey];
					$t_agr_komod_prod->setRecord($detail);
					// create detail using master table connection
					$t_agr_komod_prod->create($table->dbconn);
				}
