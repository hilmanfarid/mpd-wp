...
/**
 * d_sekolah
 * class controller for table bds_d_sekolah 
 *
 * @since 23-10-2012 12:07:20
 * @author wiliamdecosta@gmail.com
 */
class d_sekolah_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('DSekolah')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'd_sekolah.listing_no');
        $dir = wbRequest::getVarClean('dir', 'str', 'ASC');
        $query = wbRequest::getVarClean('query', 'str', '');
        
        $d_sekolah_d_sekolah_id = wbRequest::getVarClean('d_sekolah_id', 'int', 0);
		$code = wbRequest::getVarClean('code', 'str', '');
		$sekolah_name = wbRequest::getVarClean('sekolah_name', 'str', '');
		$alias_name = wbRequest::getVarClean('alias_name', 'str', '');
        $type_id = wbRequest::getVarClean('type_id', 'int', 0);
		$type_code = wbRequest::getVarClean('type_code', 'str', '');
        $level_id = wbRequest::getVarClean('level_id', 'int', 0);
        $status_id = wbRequest::getVarClean('status_id', 'int', 0);
		$status_code = wbRequest::getVarClean('status_code', 'str', '');
		$tgl_berdiri = wbRequest::getVarClean('tgl_berdiri', 'date', '');
		$address_1 = wbRequest::getVarClean('address_1', 'str', '');
		$address_2 = wbRequest::getVarClean('address_2', 'str', '');
		$kecamatan_id = wbRequest::getVarClean('kecamatan_id', 'int', 0);
		$kota = wbRequest::getVarClean('kota', 'str', '');
		$kode_pos = wbRequest::getVarClean('kode_pos', 'str', '');
		$listing_no = wbRequest::getVarClean('listing_no', 'int', 0);
        ...
        $levelSekolah = wbRequest::getVarClean('levelSekolah', 'str', '');
        
        $searchSekolah = wbRequest::getVarClean('searchSekolah', 'str', '');
        
        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 'd_sekolah');
            ...
            if(!empty($levelSekolah)) {
                $table->setCriteria('school_level.code ILIKE ?', array($levelSekolah));    
            }
            
            if(!empty($searchSekolah)) {
                $table->setCriteria('(d_sekolah.code ILIKE ? OR d_sekolah.sekolah_name ILIKE ?)', array('%'.$searchSekolah.'%','%'.$searchSekolah.'%'));    
            }
