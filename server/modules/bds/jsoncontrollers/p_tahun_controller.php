...
/**
 * p_tahun
 * class controller for table bds_p_tahun 
 *
 * @since 01-11-2012 10:52:31
 * @author agung.hp
 */
class p_tahun_controller extends wbController{    
    /**
     * read
    ...
    public static function read($args = array()){
        // Security check
        if (!wbSecurity::check('PTahun')) return;
        ...
        $sort = wbRequest::getVarClean('sort', 'str', 'tahun_id');
        $dir = wbRequest::getVarClean('dir', 'str', 'DESC');
        $query = wbRequest::getVarClean('query', 'str', '');
        
        $tahun_id = wbRequest::getVarClean('tahun_id', 'int', 0);
		$tahun_aktif = wbRequest::getVarClean('tahun_aktif', 'str', '');
		$tahun_creation_date = wbRequest::getVarClean('tahun_creation_date', 'date', '');
		$tahun_creation_by = wbRequest::getVarClean('tahun_creation_by', 'str', '');
		$tahun_updated_date = wbRequest::getVarClean('tahun_updated_date', 'date', '');
		$tahun_updated_by = wbRequest::getVarClean('tahun_updated_by', 'str', '');

        $data = array('items' => array(), 'total' => 0, 'success' => false, 'message' => '');
        try{
            $table =& wbModule::getModel('bds', 'p_tahun');
